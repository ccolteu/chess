...
class MoveCodecTest {
  ...
  @Test
  fun corruptString_isEmptyList() {
    assertEquals(emptyList<Move>(), MoveCodec.decodeAll("not-a-move"))
    assertEquals(emptyList<Move>(), MoveCodec.decodeAll("e2,e4;zz"))
    assertEquals(emptyList<Move>(), MoveCodec.decodeAll("e2,e4,NOT_A_PIECE"))
  }
}
